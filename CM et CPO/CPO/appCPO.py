...
def get_topic_contamination(cpo_id: int) -> str:
    return f"FormaReaEDF/CPO/CPO_ID{cpo_id}/NivContamination"

def get_topic_bdf(cpo_id: int) -> str:
    return f"FormaReaEDF/CPO/CPO_ID{cpo_id}/BruitDeFond"
...
def on_message(client, userdata, msg):
    try:
        ...
        if "/CPO_ID" not in msg.topic:
            return

        try:
            cpo_part = msg.topic.split("/")[2]  # CPO_ID1
            cpo_id = int(cpo_part.replace("CPO_ID", ""))
        except Exception:
            return
