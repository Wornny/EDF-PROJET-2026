// Script de la page CPO.
// Gère la jauge et l'envoi des valeurs de contamination au serveur.
...
const rawMaxCpo = Number(document.body.dataset.maxCpo || "6");
const maxCpo = Number.isInteger(rawMaxCpo) && rawMaxCpo > 0 ? rawMaxCpo : 6;

const RAW_MAX = 10000;
let lastSentContaminationText = null;
const P0 = 0;
const P10 = 2000;    // valeur 10 à 20% de la jauge
const P100 = 4000;   // valeur 100 à 40%
const P1000 = 6000;  // valeur 1000 à 60%
const P3000 = 7000;  // valeur 3000 à 70% — seuil rouge
const P_MAX = RAW_MAX; // valeur 10000 à 100%
...
function sliderToValue(position) {
  ...
  if (p <= P100) {
    ...
    return Math.pow(10, 1 + t); // 10 → 100
  }

  if (p <= P1000) {
    const t = (p - P100) / (P1000 - P100);
    return Math.pow(10, 2 + t); // 100 → 1000
  }

  if (p <= P3000) {
    const t = (p - P1000) / (P3000 - P1000);
    const logVal = Math.log10(1000) + t * (Math.log10(3000) - Math.log10(1000));
    return Math.pow(10, logVal); // 1000 → 3000
  }

  const t = (p - P3000) / (P_MAX - P3000);
  const logVal = Math.log10(3000) + t * (Math.log10(10000) - Math.log10(3000));
  return Math.pow(10, logVal); // 3000 → 10000
}
...
function setValueBoxColor(boxEl, valueNum) {
  ...
  if (valueNum < TH_YELLOW) {
    boxEl.classList.add("value-green");
  } else if (valueNum < TH_RED) {
...
function setGaugeTone(valueNum) {
  const toneClass = valueNum < TH_YELLOW
    ? "gauge-tone-green"
    : valueNum < TH_RED
      ? "gauge-tone-yellow"
      : "gauge-tone-red";
...
function sendContaminationIfNew(raw) {
  const contaminationText = formatValue(sliderToValue(raw));
  if (contaminationText === lastSentContaminationText) return;
  lastSentContaminationText = contaminationText;
...
function enableGaugePointer(gaugeBgEl, sliderEl, onRawChange) {
  ...
  const onPointerUpOrCancel = (event) => {
    ...
    sendContaminationIfNew(snap(Number(sliderEl ? sliderEl.value : 0)));
  };
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (hasEquipment && jauge) {
    ...
    jauge.addEventListener("change", (event) => {
      updateGauge(Number(event.currentTarget.value));
      sendContaminationIfNew(snap(Number(event.currentTarget.value)));
    });
      ...
      deleteModal.innerHTML = `
        <div class="cm-modal-card" role="dialog" aria-modal="true" aria-labelledby="cpo-delete-title">
          ...
          <div class="cm-modal-row" style="display: flex; gap: 10px; justify-content: flex-end; margin-top: 14px;">
            <button class="cm-modal-submit btn-cancel" id="cpo-delete-cancel" type="button">Annuler</button>
            <button class="cm-modal-submit btn-danger" id="cpo-delete-confirm" type="button">Supprimer</button>
          </div>
        </div>
      `;
