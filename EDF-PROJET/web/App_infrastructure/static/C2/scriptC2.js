// Script C2.
// Gère l'état des capteurs FACE/DOS, les interactions de la zone corporelle et l'appel au serveur.
...
    let C2_ID = document.body.dataset.c2Id || 'C2_1';
	const apiBase = '/C2';
	const MAX_C2 = Math.max(1, parseInt(document.body.dataset.maxC2 || '4', 10)) || 4;
...
            const scale = Math.min(vw / BASE_WIDTH, (vh - bannerH) / BASE_HEIGHT);
...
    });
	...
	const buildAssignedC2Ids = () => {
		...
		links.forEach((link) => {
			...
			if (Number.isInteger(id) && id >= 1 && id <= MAX_C2) {
				ids.add(id);
			}
		});
		return ids;
	...
	const refreshAddButtonVisibility = () => {
		...
		const hasFreeSlot = assigned.size < MAX_C2;
	...
	const openModal = () => {
		...
		if (modalId && modalId.options.length === 0) {
			setError(`Tous les IDs 1 a ${MAX_C2} sont deja utilises.`);
			return;
		}
		modalInput.focus();
	};
	...
	if (modalSubmit) {
		modalSubmit.addEventListener('click', () => {
			...
			if (!Number.isInteger(selectedId) || selectedId < 1 || selectedId > MAX_C2) {
				setError(`Selectionne un ID valide (1 a ${MAX_C2}).`);
				modalId?.focus();
				return;
			}
			setError('');
...
<button class="c2-modal-submit btn-cancel" id="c2-delete-cancel" type="button">Annuler</button>
					<button class="c2-modal-submit btn-danger" id="c2-delete-confirm" type="button">Supprimer</button>
					</div>
				</div>
			`;
