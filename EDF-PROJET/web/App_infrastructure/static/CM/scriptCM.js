// Script de la page Controller Mobile.
// Gère les jauges de contamination, bruit de fond et le statut, ainsi que l'envoi serveur.
...
const topPanel = gaugeBg ? gaugeBg.closest(".detector-panel") : null;
...
const rawMaxCm = Number(document.body.dataset.maxCm || "11");
const maxCm = Number.isInteger(rawMaxCm) && rawMaxCm > 0 ? rawMaxCm : 11;
...
function updateGauge(rawValue) {
  ...
  const valueText = formatValue(valueNum * 0.05);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (sendBtn) {
    const sendPressStart = (event) => {
      ...
      if (topPanel && gaugeBg) {
        const tone = ["gauge-tone-green", "gauge-tone-yellow", "gauge-tone-red"].find(c => gaugeBg.classList.contains(c)) || "gauge-tone-green";
        topPanel.classList.remove("panel-tone-green", "panel-tone-yellow", "panel-tone-red");
        topPanel.classList.add(tone.replace("gauge-tone-", "panel-tone-"));
      }
      ...
      deleteModal.innerHTML = `
        <div class="cm-modal-card" role="dialog" aria-modal="true" aria-labelledby="cm-delete-title">
          ...
          <div class="cm-modal-row" style="display: flex; gap: 10px; justify-content: flex-end; margin-top: 14px;">
            <button class="cm-modal-submit btn-cancel" id="cm-delete-cancel" type="button">Annuler</button>
            <button class="cm-modal-submit btn-danger" id="cm-delete-confirm" type="button">Supprimer</button>
          </div>
        </div>
      `;
