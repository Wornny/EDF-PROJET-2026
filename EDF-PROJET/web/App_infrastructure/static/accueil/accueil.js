// Script de navigation du carousel de la page d'accueil.
// Gère la sélection, la mémorisation locale et la navigation suivant le périphérique choisi.
...
const ACCUEIL_SELECTED_DEVICE_KEY = 'accueil.selectedDeviceLabel';
const LEGACY_MENU_SELECTED_DEVICE_KEY = 'menu.selectedDeviceLabel';
...
function getSavedIndex() {
  try {
    const savedLabel =
      localStorage.getItem(ACCUEIL_SELECTED_DEVICE_KEY) ||
      localStorage.getItem(LEGACY_MENU_SELECTED_DEVICE_KEY);
...
function getRoute(label) {
  ...
  if (label === 'Initialisateur') return '/initialisateur/index.html';
...
function updateCarousel() {
  devices.forEach((device, i) => {
    ...
    device.classList.remove('slot-left', 'slot-center', 'slot-right', 'slot-top', 'slot-back', 'back');
    ...
    else if (rel === 2 || rel === -2) device.classList.add('slot-top');
    else device.classList.add('slot-back');

    const isBack = Math.abs(rel) > 2;
  ...
  try {
    ...
    if (currentLabel) {
      localStorage.setItem(ACCUEIL_SELECTED_DEVICE_KEY, currentLabel);
    }
  } catch {
