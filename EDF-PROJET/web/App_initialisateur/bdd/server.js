...
app.use(cors({
  origin: [
    ...
    'http://192.168.10.3:55001',
    'https://simulateuredf-btslrq.fr',
    'http://simulateuredf-btslrq.fr',
...
app.delete('/api/attestation/by-code/:code', (req, res) => {
  const code = normalizeCode(decodeURIComponent(req.params.code));
  db.query('DELETE FROM init_attestation WHERE id_attestation = ?', [code], (err, result) => {
    if (err) return res.status(500).json({ error: err.message });
    if (result.affectedRows === 0) return res.status(404).json({ error: 'Attestation introuvable' });
    console.log(`🗑️ Attestation supprimée: "${code}"`);
