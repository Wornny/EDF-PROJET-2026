...
function getBarriereKey() {
  const val = document.getElementById('initSelect').value || 'initialisateur1';
  return 'barriere_etat_' + val;
}
...
let barrierEtat    = localStorage.getItem(getBarriereKey()) || 'fermee';
let resetEtat      = 0;
...
document.getElementById('btnBarrier').addEventListener('click', () => {
  ...
  localStorage.setItem(getBarriereKey(), barrierEtat);
  afficherBarriere();
...
document.querySelector('#page-index .plus').addEventListener('click', () => {
  showPrompt('Numéro de l\'initialisateur ?', (num) => {
    ...
    // Vérifie aussi les initialisateurs par défaut
    if (
        num === '1' ||
        num === '2' ||
        saved.includes(nom)
    ) {
        showAlert('Cet initialisateur existe déjà.');
        return;
    }
...
// Met à jour le bouton barrière quand on change d'initialisateur
document.getElementById('initSelect').addEventListener('change', () => {
  barrierEtat = localStorage.getItem(getBarriereKey()) || 'fermee';
  updateBarrierBtn();
});
...
function afficherBarriere() {
  showPage('barriere');
  const estOuverte   = localStorage.getItem(getBarriereKey()) === 'ouverte';
...
function refreshAttestationSelectList() {
  ...
  if (!select) return;
  select.innerHTML = '<option value="">— Sélectionner une autorisation —</option>';
  const btnDelete = document.getElementById('btnDeleteAttestation');
...
document.getElementById('btnDeleteAttestation').addEventListener('click', () => {
  const select = document.getElementById('attestationSelect');
  const code = select.value;
  if (!code) { showAlert('Sélectionne d\'abord une attestation à supprimer.'); return; }
  const label = select.options[select.selectedIndex]?.textContent || code;
  showConfirm(`🗑️ Supprimer l'autorisation :\n"${label}" ?`, (oui) => {
    if (!oui) return;
    const btn = document.getElementById('btnDeleteAttestation');
    btn.disabled = true;
    fetch(`${BASE_URL}/api/attestation/by-code/${encodeURIComponent(code)}`, { method: 'DELETE' })
      .then(r => r.json())
      .then(data => {
        if (data.success || data.deleted) {
          showAlert('🗑️ Autorisation supprimée !', () => {
            resetAttestation();
            refreshAttestationSelectList();
          });
        } else {
          showAlert('❌ Erreur suppression : ' + (data.error || 'inconnue'));
          btn.disabled = false;
        }
      })
      .catch(err => {
...
function refreshLabels() {
  ...
  lblZone.textContent = getOn(swZone) ? 'Zone valide' : 'Zone non valide';
}
  ...
  fetchBadgeFromDB(scannedCode)
    .then(data => {
      if (data.found) {
        ...
        document.getElementById('scanHint').textContent = 'Nouvelle autorisation 🆕';
      }
