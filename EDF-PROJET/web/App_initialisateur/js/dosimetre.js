...
window.onload = function () {
    ...
    document.getElementById('batterieCheck')
        .addEventListener('change', updateLabels);

    document.getElementById('horsServiceCheck')
        .addEventListener('change', updateLabels);
...
function loadSelectedDosi() {
    ...
    if (select.value === '') {
        // Remet la page dans l'état initial
        document.getElementById('display').value = '';

        document.getElementById('batterieCheck').checked = false;
        document.getElementById('horsServiceCheck').checked = false;
...
body: JSON.stringify({ numdosi, batterie, hors_service: hors_service })
})
.then(r => r.json())
.then(data => {
...
function deleteAllDosis() {
    showConfirm(
        'Êtes-vous sûr de vouloir supprimer TOUS les dosimètres ?',
        (ok) => {
            if (!ok) return;
            ...
            .then(data => {
                ...
                // Remet l'option par défaut
                select.innerHTML =
                    '<option value="">-- Sélectionner un dosimètre --</option>';
                ...
                document.getElementById('result').innerText =
                    data.message || 'Tous les dosimètres supprimés';
...
function refreshDosiList(msg) {
    ...
    .then(data => {
        ...
        // Remet l'option par défaut
        select.innerHTML = '<option value="">-- Sélectionner un dosimètre --</option>';
...
function updateLabels() {
    const batterie = document.getElementById('batterieCheck').checked;
    const horsService = document.getElementById('horsServiceCheck').checked;

    document.getElementById('batterieLabel').textContent =
        batterie ? 'Batterie valide' : 'Batterie non valide';

    document.getElementById('visiteLabel').textContent =
        horsService ? 'En service' : 'Hors service';    
}